...
{
  $file = CFile::ResizeImageGet($arResult["DETAIL_PICTURE"]["ID"],array('width' => 1050,'height' => 10000), BX_RESIZE_IMAGE_PROPORTIONAL, true);
...
if(empty($arResult["SLIDER"]) && !empty($arResult["PREVIEW_PICTURE"]["ID"])){
  ...
  $file = CFile::ResizeImageGet($arResult["PREVIEW_PICTURE"]["ID"],array('width' => 1050,'height' => 10000), BX_RESIZE_IMAGE_PROPORTIONAL, true);
  $arResult["SLIDER"]['WIDTH'] = $file['width'];
  $arResult["SLIDER"]['HEIGHT'] = $file['height'];
  $arResult["SLIDER"]['SRC'] = $file['src'];
  $sliderCountElements++;

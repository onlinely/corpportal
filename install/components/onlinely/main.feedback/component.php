...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] <> '' && (!isset($_POST["PARAMS_HASH"]) || $arResult["PARAMS_HASH"] === $_POST["PARAMS_HASH"]))
{
	...
	if(check_bitrix_sessid())
	{
		...
		{
			if((empty($arParams["REQUIRED_FIELDS"]) || in_array("NAME", $arParams["REQUIRED_FIELDS"])) && mb_strlen($_POST["user_name"]) <= 3)
		...
		if(empty($arResult["ERROR_MESSAGE"]))
		{
			$arFields = Array(
				...
				"TITLE_FORM" => $arParams["TITLE_FORM"],
				"PAGE_FORM" => $arParams["PAGE_FORM"],
				"PAGE_LINK" => $arParams["PAGE_LINK"],
				"TEXT" => $_POST["MESSAGE"],
			);
